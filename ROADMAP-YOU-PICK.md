...
| YP0.9 | Failed — v1 image pipeline diagnosed | Operator + Sol review | Verify the published buyer experience and diagnose the failed arrangement | Listing `110590142987` passed selector label/order, distinct prices, title, and description, but v1 flattened both child front/back pairs into four group images and omitted child `product.imageUrls`; all four buyer slots were placeholders and selector switching did not update images. No attestation was created. |
| YP0.10 | Blocked — publication authorization and buyer-view verification required | Operator + Sol review | Execute and verify the quantity-zero experiment | Linked fresh v2 preflight is complete. Separately authorize publication of its exact manifest, then pass buyer-view selector, price, title/description, and image-pair verification before separately authorizing quantity-zero. |
...
| YP0.13 | Complete — linked v2 run `20260806T031253Z-65754f` | Operator + Sol review | Create a predecessor-linked version-2 read-only preflight run | Manifest SHA-256 `699498f62a426b16d5e98dc43ad8b19f8ccb12a584b98ed592c12ea8c08e7b55` records cleaned predecessor `20260804T173924Z-967292`, reached `preflight-complete` with arrangement `arrangement-v2-a5d99cd506c1d224`, all gates and five collision-absence reads passed, and all 15 ledger entries `planned/0`. No mutation occurred. |
...
Predecessor-linked version-2 read-only preflight run `20260806T031253Z-65754f` is complete at manifest SHA-256 `699498f62a426b16d5e98dc43ad8b19f8ccb12a584b98ed592c12ea8c08e7b55`. It records cleaned predecessor `20260804T173924Z-967292`. The current next action is a separately authorized publication-only resume of this exact linked manifest: `pnpm --filter sidecar ebay:pilot-you-pick-sandbox -- --manifest .local/you-pick-sandbox/20260806T031253Z-65754f/manifest.json --execute --confirm-sandbox-seller testuser_mfhsandbox`. That command has not run. Earlier unlinked v2 run `20260806T025919Z-7b806c` remains immutable historical read-only evidence but is superseded and not authorized for publication. Publication, buyer-view verification, quantity-zero, withdrawal, and cleanup have not occurred for the linked run. Quantity-zero remains blocked until publication is separately authorized and buyer-view selector, price, title/description, and exact v2 image-pair behavior pass manual verification. Database work does not begin until Phase 2; YP2.1–YP2.3 remain no-write design/review tasks, and YP2.4 is the separately authorized first shared database write.
