...
| YP0.10 | Blocked — fresh v2 read-only preflight required | Operator + Sol review | Execute and verify the quantity-zero experiment | Legacy v1 cleanup is complete. Create a fresh version-2 read-only preflight with entirely new identities before any publication authorization or quantity-zero experiment. |
...
Legacy v1 run `20260804T173924Z-967292` and listing `110590142987` are fully withdrawn, deleted, and independently verified absent, including offers `11409899010`/`11409959010`, the exact group, and both child items. The byte-identical historical manifest is `cleanup-complete` at SHA-256 `c924af76e96d691a4560f8aad02d015ce111bb2cede0a0335e241a3f1d4f8926`; `verify-absence` completed/0 is expected because attempt counts record remote mutations, while timestamps and the read-back digest prove the read-only check. The legacy cleanup blocker is closed. The current next action is preparation and execution of a fresh version-2 read-only preflight with entirely new run, group, child, manifest, and collision identities. Do not create or publish the version-2 listing in this task; publication and quantity-zero remain separately authorized later gates. Database work does not begin until Phase 2; YP2.1–YP2.3 remain no-write design/review tasks, and YP2.4 is the separately authorized first shared database write.
