...
| YP0.7 | Authorization required | Operator + Sol review | Create a fresh version-5 read-only preflight run | A new manifest passes current seller, host, policy/location, metadata, arrangement-integrity, and collision-absence gates without resolving the mutation API. |
| YP0.8 | Blocked by YP0.7 | Operator + Sol review | Execute only through group publication | Exact child items, offers, and complete group read back; one common active listing ID is recorded; execution stops at `awaiting-published-view-verification`. |
...
Commit the reviewed YP0.6 implementation and this roadmap update together, while excluding `.local/**` and `.chatgpt/codex-runs/**`. After that, YP0.7 is the next product task and still requires explicit live Sandbox authorization. Database work does not begin until Phase 2; YP2.1–YP2.3 remain no-write design/review tasks, and YP2.4 is the separately authorized first shared database write.
