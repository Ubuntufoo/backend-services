...
| YP0.10 | Blocked — source review, legacy cleanup, and fresh v2 run required | Operator + Sol review | Execute and verify the quantity-zero experiment | Review/commit the v2 source fix, separately authorize cleanup of listing `110590142987` through its unchanged v1 manifest, then create and publish a fresh v2 run before any quantity-zero experiment. |
...
YP0.9 failed buyer-view verification for run `20260804T173924Z-967292`, Sandbox listing `110590142987`. Root cause: payload arrangement v1 flattened C01/C02 front/back URLs into four group-level `imageUrls` while child inventory-item products omitted `imageUrls`; that count/order did not map one group pivot to each selector value. The version-2 source fix gives each child its ordered `[front, back]` pair and gives the group only the ordered child fronts, while preserving the v1 builder, arrangement ID, digests, and cleanup identities. Next gates, in order: review and commit this source fix; separately authorize cleanup of listing `110590142987` through the unchanged legacy-compatible manifest; verify exact absence; then create a fresh version-2 preflight/run with new identities. Never revise the failed listing in place. YP0.10 and the quantity-zero experiment remain blocked. No quantity action, withdrawal, deletion, cleanup, or absence verification has run. Database work does not begin until Phase 2; YP2.1–YP2.3 remain no-write design/review tasks, and YP2.4 is the separately authorized first shared database write.
