...
| YP0.9 | Ready — published-view attestation required | Operator | Verify the published buyer experience and write the attestation | Selector label/values/order, title, description, distinct prices, front/back image pairing/order, and shared condition all pass. Any failure routes directly to cleanup. |
...
YP0.8 completed for run `20260804T173924Z-967292` from clean commit `38495eae149885e1f324984ad120e5aadbdd86f9`. One authorized guarded CLI invocation published group `YPSBX-20260804T173924Z-967292-G` as Sandbox listing `110590142987`; `publish-group` completed with attempt count 1, and completed item, offer, and group operations were not replayed. Both retained offers reconcile to the common listing ID, and the manifest is `published=true` at `awaiting-published-view-verification`. Next: YP0.9 buyer-view verification and attestation. No quantity action, withdrawal, deletion, or cleanup has run. Database work does not begin until Phase 2; YP2.1–YP2.3 remain no-write design/review tasks, and YP2.4 is the separately authorized first shared database write.
