...
function validManifestJson(): string {
  ...
  const fixture = {
    ...
    group: {
      title: 'T',
      description: 'D',
      sharedAspects: { Brand: ['P'] },
      variantSkuSnapshot: ['C01', 'C02'],
      variesBy: {
        specifications: [{ name: 'Card', values: ['a', 'b'] }],
    ...
    children: [
      {
        slot: 'C01' as const,
        selector: { name: 'Card', value: 'a' },
        productAspects: { Card: ['a'], Brand: ['P'] },
        itemQuantity: 1,
        offerQuantity: 1,
        price: { currency: 'USD' as const, value: '10.00' },
        images: [
          { role: 'front' as const, url: 'https://example.com/1.jpg', fingerprint: 'fp1' },
          { role: 'back' as const, url: 'https://example.com/2.jpg', fingerprint: 'fp2' },
        ],
        condition: {
          condition: 'USED_VERY_GOOD' as const,
          conditionId: '4000' as const,
      ...
      {
        slot: 'C02' as const,
        selector: { name: 'Card', value: 'b' },
        productAspects: { Card: ['b'], Brand: ['P'] },
        itemQuantity: 2,
        offerQuantity: 2,
        price: { currency: 'USD' as const, value: '20.00' },
        images: [
          { role: 'front' as const, url: 'https://example.com/3.jpg', fingerprint: 'fp3' },
          { role: 'back' as const, url: 'https://example.com/4.jpg', fingerprint: 'fp4' },
        ],
        condition: {
          condition: 'USED_VERY_GOOD' as const,
          conditionId: '4000' as const,
  ...
  const manifest = executableVariationListingManifestSchema.parse({
    ...
    resources: run.childSkus.map((sku, i) => ({
      sku,
      offerId: i === 0 ? 'OFFER-1' : 'OFFER-2',
...
describe('parseVerifyArgs', () => {
  ...
  it('parses valid relative manifest path and seller', () => {
    const args = parseVerifyArgs([
      '--manifest',
      '.local/you-pick-sandbox/my-run/manifest.json',
  ...
  it('throws on absolute path', () => {
    expect(() =>
      parseVerifyArgs(['--manifest', '/abs/path/manifest.json', '--confirm-sandbox-seller', 'x'])
  ...
  it('throws on traversal', () => {
    expect(() =>
      parseVerifyArgs([
        '--manifest',
        '.local/you-pick-sandbox/../other/manifest.json',
        '--confirm-sandbox-seller',
        'x',
      ])
    ).toThrow('traversal');
  ...
  it('throws on path outside guarded prefix', () => {
    expect(() =>
      parseVerifyArgs(['--manifest', 'other-dir/manifest.json', '--confirm-sandbox-seller', 'x'])
    ).toThrow('.local/you-pick-sandbox/');
  });
  ...
  it('throws on wrong filename not ending in /manifest.json', () => {
    expect(() =>
      parseVerifyArgs([
        '--manifest',
        '.local/you-pick-sandbox/run/wrong.json',
        '--confirm-sandbox-seller',
        'x',
      ])
  ...
  it('throws on backslash in path', () => {
    expect(() =>
      parseVerifyArgs([
        '--manifest',
        '.local\\you-pick-sandbox\\run\\manifest.json',
        '--confirm-sandbox-seller',
        'x',
      ])
    ).toThrow('Backslashes are forbidden');
  });
  ...
  it('throws on duplicate --manifest', () => {
    expect(() =>
      parseVerifyArgs([
        '--manifest',
        '.local/you-pick-sandbox/a/manifest.json',
        '--manifest',
        '.local/you-pick-sandbox/b/manifest.json',
        '--confirm-sandbox-seller',
        'x',
      ])
  ...
  it('throws on --fixture flag', () => {
    ...
    ).toThrow('Forbidden mutation flag: --fixture');
  });
  ...
  it('throws on --execute flag', () => {
    ...
    ).toThrow('Forbidden mutation flag: --execute');
  });
  ...
  it('throws on --cleanup flag', () => {
    ...
    ).toThrow('Forbidden mutation flag: --cleanup');
  });
  ...
  it('throws on --attestation flag', () => {
    ...
    ).toThrow('Forbidden mutation flag: --attestation');
  });
  ...
  it('throws when --manifest value is empty', () => {
    expect(() => parseVerifyArgs(['--manifest', '', '--confirm-sandbox-seller', 'x'])).toThrow(
  ...
  it('read API resolved, runtime gate fails, nothing printed', async () => {
    ...
    const apiFactory = vi.fn(async () => ({
      getRuntimeSnapshot: vi.fn(async () => {
        throw new Error('bypass runtime gate');
      }),
        ...
        readApiFactory: async () => {
          ...
          return {
            ...
            getInventoryItem: vi.fn(async (sku: string) => {
              ...
              const slot = `C0${idx + 1}`;
              const itemPayload = plan.operations.find(
                (o: any) => o.id === `item-${slot}`
              )!.payload;
              return {
                status: 'found',
                value: {
                  ...
                  quantity: manifest.ownership.itemQuantities[idx],
            ...
            getOffers: vi.fn(async (sku: string) => {
              ...
              const offerPayload = plan.operations.find(
                (o: any) => o.id === `offer-${slot}`
              )!.payload;
              return {
                status: 'found',
                value: {
                  offers: [
                    {
                      offerId: manifest.resources[idx].offerId,
                      sku,
                      marketplaceId: 'EBAY_US',
                      status: 'PUBLISHED',
                      listingId: 'LISTING-1',
                      listingStatus: 'ACTIVE',
                      lifecycleClass: 'active' as any,
            ...
            getInventoryItemGroup: vi.fn(async () => ({
              ...
              value: {
                ...
                snapshotDigest: digest(groupPayload),
              },
            })),
          };
        },
  ...
  it('throws on symlink escape via realpath', async () => {
    ...
    await expect(
      runVerifyVariationListingSandboxCli(
        ...
        {
          repoRoot: root,
          readFileImpl: async () => validManifestJson(),
          sha256FileImpl: async () => 'e'.repeat(64),
        }
      )
        ...
        ],
        {
          ...
          readApiFactory: async () => ({
            ...
            getInventoryItem: vi.fn(async () => ({
              ...
              value: { sku: 'x', groupKeys: ['G'], quantity: 1, semanticSnapshot: undefined },
            })),
