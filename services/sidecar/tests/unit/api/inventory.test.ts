...
} from '../../../src/ebay/publish-mappers.js';

function createListingForOfferPayload() {
  return {
    ...
    offer_status: null,
    price: 24.5,
    primary_image_url: 'https://example.com/image-1.jpg',
    quantity: 1,
    scheduled_for: null,
    ...
    user_id: 'user-1',
    weight_oz: 12,
  } satisfies Parameters<typeof mapListingToOfferPayload>[0];
}
...
function createListingForInventoryPayload() {
  return {
    approved_for_export_at: '2026-05-24T12:00:00.000Z',
    capture_mode: null,
    category_id: '1234',
    ...
    user_id: 'user-1',
    weight_oz: 12,
  } satisfies Parameters<typeof mapListingToInventoryItemPayload>[0];
}

function createAppSettingsForInventoryPayload() {
  return {
    created_at: '2026-05-24T09:00:00.000Z',
    ...
    updated_at: '2026-05-24T09:30:00.000Z',
    user_id: 'user-1',
  } satisfies Parameters<typeof mapListingToInventoryItemPayload>[1];
}
...
describe('InventoryApi', () => {
  ...
  describe('createOrReplaceInventoryItem', () => {
    ...
    it('should accept payload generated by mapListingToInventoryItemPayload', async () => {
      const inventoryItem = mapListingToInventoryItemPayload(
        createListingForInventoryPayload(),
        createAppSettingsForInventoryPayload()
      );
      vi.mocked(client.put).mockResolvedValue(undefined);
    ...
    it('should reject missing inventory item before client call', async () => {
      await expect(api.createOrReplaceInventoryItem('TEST-SKU', undefined as any)).rejects.toThrow(
        'inventoryItem is required and must be an object'
      );
      expect(client.put).not.toHaveBeenCalled();
    });

    it('should reject null inventory item before client call', async () => {
      await expect(api.createOrReplaceInventoryItem('TEST-SKU', null as any)).rejects.toThrow(
        'inventoryItem is required and must be an object'
      );
      expect(client.put).not.toHaveBeenCalled();
    });

    it('should reject non-object inventory item before client call', async () => {
      await expect(api.createOrReplaceInventoryItem('TEST-SKU', 'invalid' as any)).rejects.toThrow(
        'inventoryItem is required and must be an object'
      );
      expect(client.put).not.toHaveBeenCalled();
    });

    it('should wrap client failures with adapter error context', async () => {
      const inventoryItem = {
        product: { title: 'Test Product' },
        condition: 'NEW' as const,
      };
      vi.mocked(client.put).mockRejectedValue(new Error('eBay unavailable'));

      await expect(api.createOrReplaceInventoryItem('TEST-SKU', inventoryItem)).rejects.toThrow(
        'Failed to create or replace inventory item: eBay unavailable'
      );
    });
  });
  ...
  describe('createOffer', () => {
    ...
    it('should accept payload generated by mapListingToOfferPayload', async () => {
      const mockResponse = { offerId: 'OFFER123' };
      const offer = mapListingToOfferPayload(
        createListingForOfferPayload(),
        createAppSettingsForOfferPayload(),
        'SKU-001'
      );
      vi.mocked(client.post).mockResolvedValue(mockResponse);
    ...
    it('should reject missing offer payload before client call', async () => {
      await expect(api.createOffer(undefined as any)).rejects.toThrow(
        'offer is required and must be an object'
      );
      expect(client.post).not.toHaveBeenCalled();
    });

    it('should reject null offer payload before client call', async () => {
      await expect(api.createOffer(null as any)).rejects.toThrow(
        'offer is required and must be an object'
      );
      expect(client.post).not.toHaveBeenCalled();
    });

    it('should reject non-object offer payload before client call', async () => {
      await expect(api.createOffer('invalid' as any)).rejects.toThrow(
        'offer is required and must be an object'
      );
      expect(client.post).not.toHaveBeenCalled();
    });

    it('should wrap client failures with create-offer context', async () => {
      const offer = {
        sku: 'TEST-SKU',
        marketplaceId: 'EBAY_US',
        format: 'FIXED_PRICE' as const,
      ...
      vi.mocked(client.post).mockRejectedValue(new Error('eBay unavailable'));

      await expect(api.createOffer(offer)).rejects.toThrow(
        'Failed to create offer: eBay unavailable'
      );
    });
  });
