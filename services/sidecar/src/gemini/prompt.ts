...
export function buildGenerateListingDraftPrompt(input: GenerateListingDraftInput): string {
  return [
    ...
    'Listing title must be <= 76 characters and use only: player name, exact year only when visible qualifying text supports it, manufacturer, card number prefixed with the # symbol (for example, #138), and explicit market-relevant characteristics visible on the card (e.g., Rookie Card, Refractor, parallel, insert type e.g. "Grand Slammers" or "Legends", serial numbered).',
