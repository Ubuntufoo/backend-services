...
export function buildGenerateListingDraftPrompt(input: GenerateListingDraftInput): string {
  return [
    ...
    'Listing title must be <= 76 characters and use only: player name, exact year when authorized by structured userHints.explicitYear or visible qualifying text, manufacturer, card number prefixed with the # symbol (for example, #138), and explicit market-relevant characteristics visible on the card (e.g., Rookie Card, Refractor, parallel, insert type e.g. "Grand Slammers" or "Legends", serial numbered).',
    ...
    'Place the canonical title year between the player name and manufacturer or set name when that ordering is natural.',
    ...
    'For sports singles, candidate fields are: Player, Sport, Manufacturer, Set, Card Number, Parallel/Variety, Insert Set, Franchise, Features.',
    ...
    'Do not generate Season, League, Autographed, Original/Licensed Reprint, Vintage, Illustrator, Featured Person/Artist, Movie, TV Show, Genre, HP, Stage, Attribute, MTG Color, Card Size, Material, Country of Origin, Age Level, Card Thickness, MPN, UPC, or generic Graded item specifics.',
