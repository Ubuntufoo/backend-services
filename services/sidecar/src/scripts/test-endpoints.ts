...
class EndpointTester {
  ...
  async initialize(): Promise<void> {
    ...
    if (forceSandbox) {
      ...
      console.log(
        '   Set EBAY_ENVIRONMENT=sandbox in backend-services/.env to suppress this warning\n'
      );
    }
    ...
    if (!config.clientId || !config.clientSecret) {
      throw new Error(
        'Missing eBay credentials. Please set EBAY_CLIENT_ID and EBAY_CLIENT_SECRET in backend-services/.env'
      );
    }
