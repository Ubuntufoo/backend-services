...
describe('variation listing capture source configuration', () => {
  it('returns the exact configured station key and fails closed on malformed or missing values', () => {
    expect(readVariationListingCaptureSourceKey({ WATCHER_CAPTURE_SOURCE_KEY: 'station-main' })).toBe('station-main');
    expect(readVariationListingCaptureSourceKey({})).toBeNull();
    expect(() => readVariationListingCaptureSourceKey({ WATCHER_CAPTURE_SOURCE_KEY: ' station-main' })).toThrow(/outer-trimmed/);
    expect(() => requireVariationListingCaptureSourceKey({})).toThrow(/required for variation-listing intake/);
  });
});
