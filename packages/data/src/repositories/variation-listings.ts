...
export interface VariationListingIntakeSession { captureSourceKey: string; mode: string; pendingPair: Record<string, unknown> | null; source: VariationListingIntakeSessionRow; targetGroupId: string | null; targetVariationId: string | null; }
...
export function mapVariationListingIntakeSessionRow(row: VariationListingIntakeSessionRow): VariationListingIntakeSession { const pending = validateVariationListingPendingPair(row.pending_pair); return { captureSourceKey: row.capture_source_key, mode: row.mode, pendingPair: pending as Record<string, unknown> | null, source: row, targetGroupId: row.target_group_id, targetVariationId: row.target_variation_id }; }
