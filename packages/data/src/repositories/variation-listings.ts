...
export interface VariationListingIntakeSession { captureSourceKey: string; copyConditionToken: VariationListingCopyConditionToken | null; mode: 'idle' | 'new_variation' | 'duplicate_copy'; pendingPair: Record<string, unknown> | null; source: VariationListingIntakeSessionRow; stickyPriceAmount: VariationListingManualPriceAmount; stickyPriceCurrency: 'USD'; targetGroupId: string | null; targetVariationId: string | null; }
...
export function validateVariationListingPendingPair(value: Json | null): Record<string, Json> | null {
  ...
  if (mode === 'new_variation') {
    if (pair.condition_token !== null) throw new Error('Variation listing new-variation pending pair condition_token must be null.');
  } else if (!isVariationListingCopyConditionToken(pair.condition_token)) {
    throw new Error('Variation listing duplicate-copy pending pair condition_token is invalid.');
  }
...
export function mapVariationListingIntakeSessionRow(row: VariationListingIntakeSessionRow): VariationListingIntakeSession { const pending = validateVariationListingPendingPair(row.pending_pair); if (row.mode !== 'idle' && row.mode !== 'new_variation' && row.mode !== 'duplicate_copy') throw new Error('Variation listing intake session mode is invalid.'); if (!isVariationListingManualPriceAmount(row.sticky_price_amount)) throw new Error('Variation listing intake session sticky_price_amount is invalid.'); if (row.sticky_price_currency !== 'USD') throw new Error('Variation listing intake session sticky_price_currency must be USD.'); const copyConditionToken = row.copy_condition_token === null ? null : isVariationListingCopyConditionToken(row.copy_condition_token) ? row.copy_condition_token : (() => { throw new Error('Variation listing intake session copy_condition_token is invalid.'); })(); if ((row.mode === 'idle' || row.mode === 'new_variation') && copyConditionToken !== null) throw new Error('Variation listing non-duplicate intake session cannot carry copy condition.'); if (row.mode === 'duplicate_copy' && copyConditionToken === null) throw new Error('Variation listing duplicate-copy intake session requires copy condition.'); if (pending !== null && (pending.mode !== row.mode || (pending.mode === 'new_variation' && pending.condition_token !== null) || (pending.mode === 'duplicate_copy' && pending.condition_token !== copyConditionToken))) throw new Error('Variation listing pending pair condition does not match intake session.'); return { captureSourceKey: row.capture_source_key, copyConditionToken, mode: row.mode, pendingPair: pending as Record<string, unknown> | null, source: row, stickyPriceAmount: row.sticky_price_amount, stickyPriceCurrency: row.sticky_price_currency, targetGroupId: row.target_group_id, targetVariationId: row.target_variation_id }; }
