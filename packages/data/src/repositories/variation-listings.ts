...
export interface VariationListingVariation { groupId: string; position: number; representativeCopyId: string | null; selectorValue: string; sku: string; source: VariationListingVariationRow; variationId: string; }
...
export interface VariationListingIntakeSession { captureSourceKey: string; mode: 'idle' | 'new_variation' | 'duplicate_copy'; pendingPair: Record<string, unknown> | null; source: VariationListingIntakeSessionRow; stickyPriceAmount: number; stickyPriceCurrency: 'USD'; targetGroupId: string | null; targetVariationId: string | null; }
...
export function validateVariationListingPendingPair(value: Json | null): Record<string, Json> | null {
  ...
  if (![0.99, 1.49, 1.99, 2.49].includes(price)) throw new Error('Variation listing pending pair price_amount is invalid.');
...
export function mapVariationListingVariationRow(row: VariationListingVariationRow): VariationListingVariation { return { groupId: row.group_id, position: row.position, representativeCopyId: row.representative_copy_id, selectorValue: row.selector_value, sku: row.sku, source: row, variationId: row.variation_id }; }
