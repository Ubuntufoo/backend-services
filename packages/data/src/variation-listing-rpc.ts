...
import type { AbandonUntouchedVariationListingGroupInput, AdvanceVariationListingCleanupLifecycleInput, AppendVariationListingJournalCheckpointInput, AppendVariationListingJournalCheckpointResult, ApplyVariationListingGroupReviewDraftInput, MarkVariationListingPublishReadyInput, ReserveVariationListingActionRevisionInput, ReturnVariationListingToReviewInput, UpdateVariationListingManualPriceInput, UpdateVariationListingSelectorValueInput, UpdateVariationListingCopyAvailabilityInput, UpdateVariationListingRepresentativeCopyInput, CaptureVariationListingRevisionInput, CaptureVariationListingRevisionResult, ConfirmVariationListingRevisionInput, CreateVariationListingGroupInput, ConfigureVariationListingIntakeInput, StartVariationListingIntakePairInput, CompleteVariationListingNewVariationInput, CompleteVariationListingDuplicateCopyInput, VariationListingAggregateSnapshot, VariationListingTransactionGateway } from './variation-listing-transactions.js';
...
const parseSession = (v: Json): VariationListingIntakeSessionRow => {
  const r = record(v, 'session');
  str(r, 'capture_source_key');
  const mode = str(r, 'mode');
  if (!['idle', 'new_variation', 'duplicate_copy'].includes(mode)) {
    throw new Error('Variation listing RPC session mode is invalid.');
  }
  if (r.target_group_id !== null) str(r, 'target_group_id');
  if (r.target_variation_id !== null) str(r, 'target_variation_id');
  const copyCondition = r.copy_condition_token;
  if (copyCondition !== null && !isVariationListingCopyConditionToken(copyCondition)) {
    throw new Error('Variation listing RPC copy_condition_token is invalid.');
  }
  if (mode === 'idle' && (r.target_group_id !== null || r.target_variation_id !== null || copyCondition !== null)) {
    throw new Error('Variation listing RPC idle session cannot have targets or copy condition.');
  }
  if (mode === 'new_variation' && (typeof r.target_group_id !== 'string' || r.target_variation_id !== null || copyCondition !== null)) {
    throw new Error('Variation listing RPC new-variation session targets are invalid.');
  }
  if (mode === 'duplicate_copy' && (typeof r.target_group_id !== 'string' || typeof r.target_variation_id !== 'string' || !isVariationListingCopyConditionToken(copyCondition))) {
    throw new Error('Variation listing RPC duplicate-copy session targets/condition are invalid.');
  }
  const price = num(r, 'sticky_price_amount');
  if (!isVariationListingManualPriceAmount(price)) throw new Error('Variation listing RPC sticky_price_amount is invalid.');
  if (r.sticky_price_currency !== 'USD') throw new Error('Variation listing RPC sticky_price_currency must be USD.');
  const pending = validateVariationListingPendingPair(r.pending_pair as Json | null);
  if (pending !== null && (pending.mode !== mode || (pending.mode === 'new_variation' && pending.condition_token !== null) || (pending.mode === 'duplicate_copy' && pending.condition_token !== copyCondition))) {
    throw new Error('Variation listing RPC pending pair condition does not match session.');
  }
  str(r, 'created_at');
  str(r, 'updated_at');
  return r as unknown as VariationListingIntakeSessionRow;
};
...
async function abandonUntouchedGroup(client: SupabaseDataClient, input: AbandonUntouchedVariationListingGroupInput): Promise<VariationListingGroupRow> {
  if (input.expectedDesiredRevision !== 0) throw new Error('Variation listing untouched abandonment expected desired revision must be 0.');
  const row = await rpcSingle<{ group_row: Json }>(client, 'abandon_untouched_variation_listing_group', {
    p_group_id: input.groupId,
    p_expected_desired_revision: input.expectedDesiredRevision,
  }, 'Variation listing untouched abandonment returned no row.');
  const group = parseGroup(row.group_row);
  if (group.group_id !== input.groupId || group.desired_revision !== 0 || group.last_confirmed_revision !== null || group.lifecycle_state !== 'abandoned') throw new Error('Variation listing RPC response parity mismatch.');
  return group;
}
async function advanceCleanupLifecycle(client: SupabaseDataClient, input: AdvanceVariationListingCleanupLifecycleInput): Promise<VariationListingGroupRow> {
  if (!Number.isInteger(input.expectedDesiredRevision) || input.expectedDesiredRevision < 0) throw new Error('Variation listing cleanup expected desired revision must be a non-negative integer.');
  if (input.expectedPreviousConfirmedRevision !== null && (!Number.isInteger(input.expectedPreviousConfirmedRevision) || input.expectedPreviousConfirmedRevision < 1)) throw new Error('Variation listing cleanup expected previous confirmed revision must be null or a positive integer.');
  if (!['withdrawn', 'cleanup', 'abandoned', 'terminal-absent'].includes(input.targetLifecycle)) throw new Error('Variation listing cleanup target lifecycle is invalid.');
  const row = await rpcSingle<{ group_row: Json }>(client, 'advance_variation_listing_cleanup_lifecycle', {
    p_group_id: input.groupId,
    p_revision_id: input.revisionId,
    p_expected_desired_revision: input.expectedDesiredRevision,
    p_expected_previous_confirmed_revision: input.expectedPreviousConfirmedRevision,
    p_target_lifecycle: input.targetLifecycle,
  }, 'Variation listing cleanup lifecycle transition returned no row.');
  const group = parseGroup(row.group_row);
  if (group.group_id !== input.groupId || group.desired_revision !== input.expectedDesiredRevision || group.last_confirmed_revision !== input.expectedPreviousConfirmedRevision || group.lifecycle_state !== input.targetLifecycle) throw new Error('Variation listing RPC response parity mismatch.');
  return group;
}
...
async function markPublishReady(client: SupabaseDataClient, input: MarkVariationListingPublishReadyInput): Promise<VariationListingGroupRow> {
  if (!Number.isInteger(input.expectedDesiredRevision) || input.expectedDesiredRevision < 1) throw new Error('Variation listing publish-ready expected revision must be a positive integer.');
  const row = await rpcSingle<{ group_row: Json }>(client, 'mark_variation_listing_publish_ready', {
    p_group_id: input.groupId,
    p_expected_desired_revision: input.expectedDesiredRevision,
  }, 'Variation listing publish-ready transition returned no row.');
  const group = parseGroup(row.group_row);
  if (group.group_id !== input.groupId || group.desired_revision !== input.expectedDesiredRevision || group.last_confirmed_revision !== null || group.lifecycle_state !== 'publish-ready') throw new Error('Variation listing RPC response parity mismatch.');
  return group;
}
async function reserveActionRevision(client: SupabaseDataClient, input: ReserveVariationListingActionRevisionInput): Promise<VariationListingGroupRow> {
  if (!Number.isInteger(input.expectedDesiredRevision) || input.expectedDesiredRevision < 0) throw new Error('Variation listing action revision expected revision must be a non-negative integer.');
  const row = await rpcSingle<{ group_row: Json }>(client, 'reserve_variation_listing_action_revision', {
    p_group_id: input.groupId,
    p_expected_desired_revision: input.expectedDesiredRevision,
  }, 'Variation listing action revision reservation returned no row.');
  const group = parseGroup(row.group_row);
  if (group.group_id !== input.groupId || group.desired_revision !== input.expectedDesiredRevision + 1) throw new Error('Variation listing RPC response parity mismatch.');
  return group;
}
async function returnToReview(client: SupabaseDataClient, input: ReturnVariationListingToReviewInput): Promise<VariationListingGroupRow> {
  if (!Number.isInteger(input.expectedDesiredRevision) || input.expectedDesiredRevision < 1) throw new Error('Variation listing return-to-review expected revision must be a positive integer.');
  const row = await rpcSingle<{ group_row: Json }>(client, 'return_variation_listing_to_review', {
    p_group_id: input.groupId,
    p_expected_desired_revision: input.expectedDesiredRevision,
    p_cleanup_revision_id: input.cleanupRevisionId,
  }, 'Variation listing return-to-review returned no row.');
  const group = parseGroup(row.group_row);
  if (group.group_id !== input.groupId || group.desired_revision !== input.expectedDesiredRevision + 1 || group.last_confirmed_revision !== null || group.lifecycle_state !== 'review') throw new Error('Variation listing return-to-review RPC response parity mismatch.');
  return group;
}
...
async function updateVariationSelectorValue(client: SupabaseDataClient, input: UpdateVariationListingSelectorValueInput): Promise<{ group: VariationListingGroupRow; variation: VariationListingVariationRow }> {
  ...
  const row = await rpcSingle<{ group_row: Json; variation_row: Json }>(client, 'update_variation_listing_selector_value', {
    ...
    p_variation_id: input.variationId,
    p_expected_desired_revision: input.expectedDesiredRevision,
    p_selector_value: selectorValue,
  }, 'Variation listing selector edit returned no row.');
  const group = parseGroup(row.group_row);
  const variation = parseVariation(row.variation_row);
  if (group.group_id !== input.groupId || group.desired_revision !== input.expectedDesiredRevision + 1 || variation.group_id !== input.groupId || variation.variation_id !== input.variationId || variation.selector_value !== selectorValue) throw new Error('Variation listing RPC response parity mismatch.');
  return { group, variation };
}
...
export function createSupabaseVariationListingTransactionGateway(client: SupabaseDataClient): VariationListingTransactionGateway { return {
  loadAggregate: id => loadAggregate(client,id), captureRevision: i => captureRevision(client,i), appendJournalCheckpoint: i => appendJournalCheckpoint(client,i), abandonUntouchedGroup: i => abandonUntouchedGroup(client,i), advanceCleanupLifecycle: i => advanceCleanupLifecycle(client,i), confirmRevision: i => confirmRevision(client,i), applyGroupReviewDraft: i => applyGroupReviewDraft(client,i), markPublishReady: i => markPublishReady(client,i), reserveActionRevision: i => reserveActionRevision(client,i), returnToReview: i => returnToReview(client,i), updateVariationPrice: i => updateVariationPrice(client,i), updateVariationSelectorValue: i => updateVariationSelectorValue(client,i), updateCopyAvailability: i => updateCopyAvailability(client,i), updateRepresentativeCopy: i => updateRepresentativeCopy(client,i),
