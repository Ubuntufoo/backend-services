...
| 9L.1 | $DOCS$ | Repo docs need a macro review to reflect the new pricing service and its integration with the existing architecture | General doc cleanup should also be done in this pass |
...
| 9N.2 | BE/API & Jobs | API and workflow cleanup pass | Review HTTP routes, DTOs, validation, job orchestration, retry handling, workflow transitions, and error paths; remove defensive-code slop, redundant logic, stale diagnostics, and inconsistent patterns |
