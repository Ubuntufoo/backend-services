...
| 10A.1 | BE/Discovery | Confirm read-only production Browse search entitlement | Using the configured production eBay keyset, mint a client-credentials Application token and perform one sanitized, non-mutating `EBAY_US` `GET /buy/browse/v1/item_summary/search` smoke query expected to return active results. Record token success/failure, Browse HTTP status, sanitized query/result count, whether production Browse is actually enabled, the documented 5,000/day default and any effective quota exposed by Developer Analytics, and any remaining EPN/Buy API application/Developer Support/contract requirement. Do not use the ordinary user-token-preferred API client path for this entitlement test, alter credentials, expose secrets, implement Browse integration, or add buying/checkout capability. |
