...
| 9J.8 | BE/Pricing | Keep LLM as optional price reasoning only | LLM may only apply a guarded condition adjustment to the deterministic median; deterministic exact-card validation and stats remain the authority for comp eligibility and baseline price.| COMPLETED TASKS COMMENTED OUT -->
| 9J.9 | BE/Pricing | Apply search modifiers across pricing providers | Shared raw-card search modifiers, including grading-company exclusions and broad-selection keywords like `-"you pick"`, are appended to SoldComps and Apify actor requests through provider-neutral query construction |
| 9J.10 | FE | Add pricing provider dashboard selector per listing | Dashboard segmented control selects Pricing Off, SoldComps, or Apify; reflects persisted mode and handles loading, saving, disabled, and backend configuration-error states |
| 9J.11 | FE | Add keyword modifier inputs per listing | Users can specify search criteria for pricing queries that include the default "-<modifier>" keywords. This allows for more targeted and accurate pricing results for graded slabs, prizms, variants |
...
| 9J.13 | BE/Pricing | Harden price update policy | Listing price updates only after a validated positive result and successfully stored research row, regardless of selected provider |
| 9J.14 | BE/Pricing | Add provider fallback routing | Deferred: optionally fall back between SoldComps and Apify after live provider behavior is understood |
