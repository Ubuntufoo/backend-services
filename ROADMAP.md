...
| 8 | BE | Add production publish safety gate | Live eBay publishing requires explicit production allow flag | GPT-5.5 mini | Low-Medium | Require `EBAY_ENVIRONMENT=production` plus `EBAY_ALLOW_PRODUCTION_PUBLISH=true`; block before eBay API call otherwise |
