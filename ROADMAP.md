...
| 8 | BE | Add best-effort pricing research foundation | After Gemini, single-card listings can receive sold-comps pricing without blocking review/export flow | GPT-5.5 mini | Medium | Sidecar module only; add research_price job, pricing table/repository, fixture provider, Apify env flags, no hard app-level Apify call cap, minimum 12 sold comps, update listings.price on success, keep pricing failures isolated from listing workflow |
...
| 8 | BE/FE | Remove 'assets_ready' (seller hints) step for singles | Single listings skip AI generation and go straight from watcher intake to review | GPT-5.5 mini | Medium | Simplifies single listing flow for pilot; lot listings keep full flow with seller hints and Gemini draft |
...
| 8 | BE | `ebay-service`: implement `getUnshippedOrders()` | Order checks work against eBay API | GPT-5.5 | Medium | Begin post-listifng order workflow |
