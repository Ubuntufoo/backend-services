...
| 9J.8 | BE/Pricing | Keep LLM as optional price reasoning only | LLM may only apply a guarded condition adjustment to the deterministic median; deterministic exact-card validation and stats remain the authority for comp eligibility and baseline price.| 
| 9J.9 | BE/Pricing | Apply deterministic raw-card shipping defaults | Raw-card single comp totals ignore unreliable provider shipping and use price-bucket defaults: `$0.00-$2.99 -> $1.00`, `$3.00-$7.99 -> $1.25`, `$8.00-$14.99 -> $2.00`, `$15.00-$19.99 -> $2.75`; `$20+` remains deferred until high-value single shipping behavior is understood |
| 9J.10 | BE/Pricing | Apply search modifiers across pricing providers | Shared raw-card search modifiers, including grading-company exclusions and broad-selection keywords like `-"you pick"`, are appended to SoldComps and Apify actor requests through provider-neutral query construction |
| 9J.11 | FE | Add pricing provider dashboard selector per listing | Dashboard segmented control selects Pricing Off, SoldComps, or Apify; reflects persisted mode and handles loading, saving, disabled, and backend configuration-error states | 
